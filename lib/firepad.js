...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.monitorCursors_ = function() {
    ...
    function childChanged(childSnap) {
      ...
      //if(typeof userData.cursor !== "undefined" ){
        var popOver = document.getElementById(userId);
        alog(popOver);
...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherCursor = function (cursor, color, clientId) {
    ...
    if (cursor.position === cursor.selectionEnd) {
      ...
      var codeMirrorCode = document.querySelector(".CodeMirror-code");
      alog(codeMirrorCode);
      codeMirrorCode.appendChild( spanName );
      //document.body.appendChild( spanName );
      spanName.style.position = "absolute"; //absolute, fixed
      spanName.id = clientId;
      spanName.innerHTML = "Teacher html";
      ...
      if(cursorPos.line == 0){
        ...
        spanName.style.top = cursorEl.style.height;
        //spanName.style.marginTop = cursorEl.style.height;
      }else{
        alog("cursorEl.style.height = " + cursorEl.style.height);
        spanName.style.top = "-" + (parseInt(cursorEl.style.height)+5) + "px"; //padding 위 아래 숫자도 더해주기
