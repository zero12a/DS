...
require_once('rdfludntdService.php');
...
}else if($objAuth->isAuth("RDFLUDNTD",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"RDFLUDNTD",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"RDFLUDNTD",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new rdfludntdService();
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("RdfludntdControl___________________________end");
$log->close(); unset($log);
?>
