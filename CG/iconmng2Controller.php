...
require_once('iconmng2Service.php');
...
$log->info("Iconmng2Control___________________________start");
...
}else if($objAuth->isAuth("ICONMNG2",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"ICONMNG2",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"ICONMNG2",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new iconmng2Service();
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("Iconmng2Control___________________________end");
$log->close(); unset($log);
?>
