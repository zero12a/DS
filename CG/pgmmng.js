...
,				{ "COLID": "GRPTYPES", "COLNM" : "GRPTYPES", "OBJTYPE" : "DROPDOWN" }
,				{ "COLID": "OBJTYPES", "COLNM" : "OBJTYPES", "OBJTYPE" : "DROPDOWN" }
,				{ "COLID": "ADDDT", "COLNM" : "ADDDT", "OBJTYPE" : "INPUTBOX" }
,				{ "COLID": "MODDT", "COLNM" : "MODDT", "OBJTYPE" : "INPUTBOX" }
			]
		}
...
function G3_INIT(){
  alog("G3_INIT()-------------------------start");
	...
	//onCheck
		...
		lastinputG6.set("G3-PJTSEQ", mygridG3.cells(rowID,mygridG3.getColIndexById("PJTSEQ")).getValue().replace(/&amp;/g, "&")); // 
		lastinputG7json = jQuery.parseJSON('{ "__NAME":"lastinputG7json"' +
			', "G3-PJTSEQ" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("PJTSEQ")).getValue()) + '"' +
		'}');
		lastinputG7 = new HashMap(); // FILE
		lastinputG7.set("__ROWID",rowID);
		lastinputG7.set("G3-PJTSEQ", mygridG3.cells(rowID,mygridG3.getColIndexById("PJTSEQ")).getValue().replace(/&amp;/g, "&")); // 
...
function G5_INIT(){
  alog("G5_INIT()-------------------------start");
	...
	//헤더초기화
	mygridG5.setHeader("PJTSEQ,DDSEQ,컬럼ID,컬럼명,단축명,데이터타입,데이터사이즈,오브젝트타입,OBJ폼뷰,OBJ그리드,라벨가로,라벨세로,LBLALIGN,오브젝트가로,오브젝트세로,가로정렬,CRYPTCD,VALIDSEQ,PIYN,등록일,수정일");
...
function G6_INIT(){
  alog("G6_INIT()-------------------------start");
	...
	//헤더초기화
	mygridG6.setHeader("PJTSEQ,SEQ,사용,CFGID,CFGNM,MVCGBN,PATH,ORD,GRPTYPES,OBJTYPES,ADDDT,MODDT");
	//헤더 필터추가
	mygridG6.attachHeader(",,#text_filter,#text_filter,#text_filter,#select_filter,#text_filter,#numeric_filter,#text_filter,#text_filter,,");
	mygridG6.setColumnIds("PJTSEQ,CFGSEQ,USEYN,CFGID,CFGNM,MVCGBN,PATH,CFGORD,GRPTYPES,OBJTYPES,ADDDT,MODDT");
	mygridG6.setInitWidths("30,50,50,60,120,60,300,30,120,100,80,80");
	mygridG6.setColTypes("ed,ed,ed,ed,ed,coro,ed,ed,dropdown,dropdown,ed,ed");
	//가로 정렬	
	...
	mygridG6.attachEvent("onDhxCalendarCreated", function(myCal){ myCal.loadUserLanguage( "kr" ); });
		...
		apiCodeCombo('G6','MVCGBN',{'G1-PCD':'MVCGBN'},''); // IO : MVCGBN초기화	
		 // IO : PATH초기화	
		 // IO : ORD초기화	
		apiCodeDropDown('G6','GRPTYPES',{'G1-PCD':'GRPTYPE'},''); // IO : GRPTYPES초기화	
		apiCodeDropDown('G6','OBJTYPES',{'G1-PCD':'CTOBJTYPE'},''); // IO : OBJTYPES초기화	
		 // IO : ADDDT초기화	
		 // IO : MODDT초기화	
	//onCheck
...
function G6_EXCEL(){	
	...
	$("#DATA_HEADERS").val("PJTSEQ,CFGSEQ,USEYN,CFGID,CFGNM,MVCGBN,PATH,CFGORD,GRPTYPES,OBJTYPES,ADDDT,MODDT");
	$("#DATA_WIDTHS").val("30,50,50,60,120,60,300,30,120,100,80,80");
	$("#DATA_ROWS").val(myXmlString);
	myForm.submit();
}
//새로고침	
