...
,				{ "COLID": "LAYOUTID", "COLNM" : "LAYOUTID", "OBJTYPE" : "INPUTBOX" }
,				{ "COLID": "GRPCNT", "COLNM" : "GRPCNT", "OBJTYPE" : "INPUTBOX" }
	...
	"G3", 
		{
			...
			,"SEQYN": "N"
			,"COLS": [
				{ "COLID": "LAYOUTDSEQ", "COLNM" : "LAYOUTDSEQ", "OBJTYPE" : "INPUTBOXRO" }
,				{ "COLID": "PJTSEQ", "COLNM" : "PJTSEQ", "OBJTYPE" : "INPUTBOX" }
,				{ "COLID": "LAYOUTID", "COLNM" : "LAYOUTID", "OBJTYPE" : "INPUTBOX" }
...
,				{ "COLID": "SPLITDIRECTION", "COLNM" : "DIRECTION", "OBJTYPE" : "INPUTBOX" }
...
var lastselectG4json;//오브젝트 사이즈리셋
//사이즈 리셋 : 
function G1_RESIZE(){
...
function G2_INIT(){
  alog("G2_INIT()-------------------------start");
	...
	//onCheck
		...
		lastinputG4.set("G2-LAYOUTID", mygridG2.cells(rowID,mygridG2.getColIndexById("LAYOUTID")).getValue().replace(/&amp;/g, "&")); // 
...
function G4_INIT(){
  alog("G4_INIT()-------------------------start");
	...
	mygridG4.setUserData("","gridTitle","G4 : LAYOUTS"); //글로별 변수에 그리드 타이블 넣기
	//헤더초기화
	mygridG4.setHeader("LAYOUTSSEQ,LAYOUTID,GRPID,REFGRPID,ORD,GRPTYPE,GRPWIDTH,GRPHEIGHT,PGRPID,GUTTERSIZE,DIRECTION,MINSIZE,ADDDT,ADDID,수정일,MODID");
	mygridG4.setColumnIds("LAYOUTSSEQ,LAYOUTID,GRPID,REFGRPID,ORD,GRPTYPE,GRPWIDTH,GRPHEIGHT,PGRPID,SPLITGUTTERSIZE,SPLITDIRECTION,SPLITMINSIZE,ADDDT,ADDID,MODDT,MODID");
	mygridG4.setInitWidths("60,60,120,60,150,120,60,60,60,60,60,60,40,100,40,100");
	mygridG4.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ro,ro,ro,ro");
	//가로 정렬	
	mygridG4.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
	mygridG4.setColSorting("int,str,str,str,int,str,str,str,str,int,str,int,str,int,str,int");	//렌더링	
	mygridG4.enableSmartRendering(true);
	mygridG4.enableMultiselect(true);
	//mygridG4.setColValidators("G4_LAYOUTSSEQ,G4_LAYOUTID,G4_GRPID,G4_REFGRPID,G4_ORD,G4_GRPTYPE,G4_GRPWIDTH,G4_GRPHEIGHT,G4_PGRPID,G4_SPLITGUTTERSIZE,G4_SPLITDIRECTION,G4_SPLITMINSIZE,G4_ADDDT,G4_ADDID,G4_MODDT,G4_MODID");
...
//그리드 행추가 : LAYOUTS
	function G4_ROWADD(){
		if( !(lastinputG4)|| lastinputG4.get("G4-LAYOUTID") == ""){
			...
			var tCols = ["",lastinputG4.get("G2-LAYOUTID"),"","","","","","","","","","","","","",""];//초기값
